...
public class TestViewer extends WindowAdapter implements
		CaptureCallback {
	...
	private static final int[] pureGreen = new int[] { 0, 255, 0 };
	...
	private static final int blueThreshForYellow = 0; // not actually used but
														// could be needed at
														// some point
	...
	private void initFrameGrabber() throws V4L4JException {
		...
		try {
			...
			for (Control c2 : controls)
				System.out.println("control name: " + c2.getName() + " - min: "
						+ c2.getMinValue() + " - max: " + c2.getMaxValue()
						+ " - step: " + c2.getStepValue() + " - value: "
						+ c2.getValue());
	...
	public void nextFrame(VideoFrame frame) {
		...
		label.getGraphics().drawImage(ball_cross_drawn, 0, 0, width, height,
				null);
	...
	public BufferedImage findBallPosition(BufferedImage thisFrame) {
		...
		int redness = -1;
		// must create point this way or it does not create new object (which
		// creates errors in for loops)
		...
		if (prev_pos.x == -1 || prev_pos.x <= ball_box_radius
			...
			for (int x = width_margin; x < thisFrame.getWidth() - width_margin; x++) {
				for (int y = height_margin; y < thisFrame.getHeight()
						- height_margin; y++) {
			...
			for (int x = prev_pos.x - ball_box_radius; x < prev_pos.x
					+ ball_box_radius; x++) {
				for (int y = prev_pos.y - ball_box_radius; y < prev_pos.y
						+ ball_box_radius; y++) {
	...
	public BufferedImage detectRobots(BufferedImage image) {
		...
		// centroid connection
		int howManyGreensAroundYellow = 0;
		int howManyGreensAroundBlue = 0;
		int howManyYellows = 0;
		int howManyBlues = 0;

		Point yellowCentroid = new Point(0, 0);
		Point blueCentroid = new Point(0, 0);
		Point GreenForYellowCentroid = new Point(0, 0);
		Point GreenForBlueCentroid = new Point(0, 0);

		Point actualCentroidPosYellow = new Point(0, 0);
		Point actualCentroidPosBlue = new Point(0, 0);
		Point actualCentroidPosGreenForYellow = new Point(0, 0);
		Point actualCentroidPosGreenForBlue = new Point(0, 0);
		...
		for (int x = width_margin; x < image.getWidth() - width_margin; x++) {
			for (int y = height_margin; y < image.getHeight() - height_margin; y++) {
				...
				if (isSthGreen(colour)) {
					// drawPixel(raster, currentPoint, pureBlack);

					greenpoints.add(currentPoint);
				}
		...
		/*
		 * for (int i = 0; i < greenpoints.size(); i++) { // if
		 * (calcDistanceBetweenPoints(greenpoints.get(i), //
		 * actualCentroidPosBlue) < 150) { //
		 * System.out.println(greenpoints.get(i)); // drawPixel(raster,
		 * greenpoints.get(i), pureBlue); // } if
		 * (calcDistanceBetweenPoints(greenpoints.get(i),
		 * actualCentroidPosYellow) < 13) {
		 * 
		 * GreenForYellowCentroid.x += greenpoints.get(i).x;
		 * GreenForYellowCentroid.y += greenpoints.get(i).y;
		 * howManyGreensAroundYellow++; drawPixel(raster, greenpoints.get(i),
		 * pureGreen); }
		 * 
		 * if (calcDistanceBetweenPoints(greenpoints.get(i),
		 * actualCentroidPosBlue) < 10) {
		 * 
		 * GreenForBlueCentroid.x += greenpoints.get(i).x;
		 * GreenForBlueCentroid.y += greenpoints.get(i).y;
		 * howManyGreensAroundBlue++; drawPixel(raster, greenpoints.get(i),
		 * pureGreen); }
		 * 
		 * }
		 */
		...
		for (int i = 0; i < bluepoints.size(); i++) {
			// if (calcDistanceBetweenPoints(greenpoints.get(i),
			// actualCentroidPosBlue) < 150) {
			// System.out.println(greenpoints.get(i));
			// drawPixel(raster, greenpoints.get(i), pureBlue);
			// }

			if (calcDistanceBetweenPoints(bluepoints.get(i),
					actualCentroidPosBlue) > d) {

				d = calcDistanceBetweenPoints(bluepoints.get(i),
						actualCentroidPosBlue);
				test = bluepoints.get(i);
			}

		}

		// drawCross(raster, test, pureYellow);

		if (howManyGreensAroundYellow > 5) {
			actualCentroidPosGreenForYellow.x = GreenForYellowCentroid.x
					/ howManyGreensAroundYellow;
			actualCentroidPosGreenForYellow.y = GreenForYellowCentroid.y
					/ howManyGreensAroundYellow;
		}

		if (howManyGreensAroundBlue > 5) {
			actualCentroidPosGreenForBlue.x = GreenForBlueCentroid.x
					/ howManyGreensAroundBlue;
			actualCentroidPosGreenForBlue.y = GreenForBlueCentroid.y
					/ howManyGreensAroundBlue;
		}
		// drawCross(raster, actualCentroidPosBlue, pureYellow);
		// drawCross(raster, actualCentroidPosGreenForBlue, pureBlack);
		drawLine(raster, actualCentroidPosBlue, test, pureRed);
	...
	public boolean isSthBlue(int[] colour) {
		...
		return (colour[0] < 50 && colour[2] > 100);
	}
	...
	public boolean isSthBlack(int[] colour) {
		...
		return (blackSim[0] < redThreshForBlack
				&& blackSim[1] < greenThreshForBlack && blackSim[2] < blueThreshForBlack);
	}
	...
	public static double calcDistanceBetweenPoints(Point p1, Point p2) {
		return Math.sqrt((double) (Math.pow(p1.x - p2.x, 2) + (Math.pow(p1.y
				- p2.y, 2))));
	}
