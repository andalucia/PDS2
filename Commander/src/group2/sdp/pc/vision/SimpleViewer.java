...
public class SimpleViewer extends WindowAdapter implements CaptureCallback{
	 ...
	 */
	 ...
	 public BufferedImage normalizeFrame(BufferedImage image) {
		...
		// for every point on the image
		 ...
		 {          
			 ...
			 {   
				 ...
				 if (( (image.getRGB(x,y) >> 16) & 0xFF) > redness) {
					 //System.out.println(((thisFrame.getRGB(x,y) >> 16) & 0xFF) + " > " + redness);
					 ball_pos.x = x;
					 ball_pos.y = y;
					 redness = (image.getRGB(x,y) >> 16) & 0xFF;
				 }
		...
		// find the ball
		drawCross(raster, ball_pos, pureRed);
