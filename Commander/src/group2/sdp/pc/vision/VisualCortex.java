 ...
 * Main client: Bakery. Actions: Extracts image features, like position of the
 * ball and the robots, and orientation of the T-shapes on the top of the
 * robots. Passes that information to a Static Info Consumer that is supplied on
 * construction of the Visual Cortex.
 */
public class VisualCortex extends VisualCortexSkeleton {
	...
	// TODO: Fix comments, names. this alg is O(N^2) - potential bottleneck!
	/**
	...
	// TODO: you can do better with this comment, variable names... need
	// clarification if not replacement
	/**
	 * regression function
	 * 
	 * @param fixels
	 ...
	 * @return
	 */
	protected double regression(ArrayList<Point> fixels, double angle,
			boolean isYellow) {
	 ...
	 * 
	 * @param colour
	 *            The colour you are checking
	 * @param isYellow
	 *            If you are looking for yellow (the other option is blue)
	 * @return
	 */
	private boolean isBlueYellow(BufferedImage image, Point pixel,
			boolean isYellow) {
		boolean returnValue = false;
		// TODO check bounds: extract to a function (prob in util.Tools)
		int width = 640;
		int height = 480;
	...
	// TODO: Add comments, rename variables.
	/**
	 * 
	 * @param fixels
	 *            - for every ArrayList<Point> robot/ball, calculate its
	 *            centroid and return it
	 * @return
	 */
	...
	// TODO: p2? restructure function
	/**
	 * WARNING: DO NOT USE p2 AFTER THIS FUNCTION HAS BEEN CALLED. This function
	 * will change the values of p2. Use the returned point and create a copy of
	 * p2 if you want to use it. This will also perform very badly if
	 * continually used to rotate by 1 degrees.
	 */
	public Point rotatePoint(Point pivot, Point rotate, int deg) {
		...
		int xtemp;
		xtemp = (int) Math.round((point.x * (double) Math.cos(rad))
				- (point.y * (double) Math.sin(rad)));
		point.y = (int) Math.round((point.x * (double) Math.sin(rad))
				+ (point.y * (double) Math.cos(rad)));
		point.x = xtemp;
		return new Point(point.x + pivot.x, point.y + pivot.y);
	}

	/**
	 * Converts from the image coordinate system (in pixels) to a coordinate
	 * system centred at the physical centre of the pitch (in cm), where y grows
	 * upwards and x to the right.
	 * 
	 * @param point
	 ...
	 * is an error proportional to the distance of the robot from the centre of
	 * the pitch. This is corrected in this function. Here is a rough sketch:
	 * Camera . |\ | \ | \ H | \ |____\ <- robot centroid. | h|\ |____|_\ delta
	 * d
	 * 
	 * @param robotPosition
	 *            The pixel position of the robot.
	 * @param convertedPosition
	 *            The converted position of the robot, before correction.
	 * @param height
	...
	private Point2D correctRobotPosition(Point robotPosition,
		...
		double h = height;
		double d = robotPosition.distance(pitchImageRectangle.getCenterX(),
	...
	/**
	 * Changes the coordinate system of a 1-dimensional variable. E.g. if you
	 * want to move your starting point from 1 to 3, and to dilate your range
	 * from 3 to 6, this would move 2 to 5:
	 * 
	 * 1 2 3 4 5 6 7 8 9 Input : > . < Output: > . <
	 * 
	...
	protected Point2D extractRobotPosition(BufferedImage image, boolean yellow) {
		Point robotPosition = yellow ? yellowCentroid : blueCentroid;
		if (robotPosition == null) {
			return null;
		}
		Point2D convertedPosition = convertPixelsToCm(robotPosition);
		return correctRobotPosition(robotPosition, convertedPosition,
